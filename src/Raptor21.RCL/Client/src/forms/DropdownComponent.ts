...
export class DropdownComponent extends RaptorComponent {
    ...
    openPanel(): void {
        ...
        // The panel is portalled to <body> on mount. A host inside an hx-preserve'd subtree survives a
        // boosted navigation's body swap by reference, but the panel — a child of body rather than a
        // descendant of the preserved node — is detached by it. Re-attach by reference before
        // positioning; the node and its content are untouched, only its parent changed.
        ...
        if (this.sheetMode) {
            ...
            document.body.insertBefore(backdrop, this.panel)
