...
/// <summary>
/// Emits <c>RaptorRoutes</c> — one nested static class per <c>[RaptorPage]</c>/<c>[RaptorComponent]</c> class in
...
/// <c>hx-post="@RaptorRoutes.PageGroupModal.Edit(id, vkorgId)"</c>: the route string exists in exactly one place
...
public sealed class RaptorRoutesGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var all = pages.Collect().Combine(components.Collect());

        context.RegisterSourceOutput(all, static (spc, pair) =>
        {
            var models = pair.Left.AddRange(pair.Right);
            if (models.Length == 0) return;
            spc.AddSource("RaptorRoutes.g.cs", SourceText.From(Emit(models), Encoding.UTF8));
        });
    }
    ...
    private static string Emit(ImmutableArray<PageModel> models)
    {
        ...
        sb.AppendLine("    internal static class RaptorRoutes");
        sb.AppendLine("    {");
