...
public sealed partial class RaptorRoutesGenerator : IIncrementalGenerator
{
        ...
        SourceProductionContext spc, ImmutableArray<MarkupFileModel> files, ImmutableArray<PageModel> pages)
    {
        ...
        var pageByClass = new Dictionary<string, PageModel>(StringComparer.Ordinal);
        foreach (var page in Deduplicate(pages))
        {
            if (!pageByClass.ContainsKey(page.ClassName)) pageByClass[page.ClassName] = page;
        }
